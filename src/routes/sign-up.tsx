...
function SignUpPage() {
  ...
  return (
    <AuthLayout
      ...
      footer={<>Already have an account? <Link to="/signin" className="text-primary font-semibold hover:underline">Sign in</Link></>}
    >
      <form onSubmit={(e) => { e.preventDefault(); navigate({ to: "/dashboard" }); }} className="space-y-4">
        ...
        <AuthInput label="Password" type="password" placeholder="At least 12 characters" autoComplete="new-password" />
        ...
        <FxButton size="lg" variant="cyber" className="w-full">Create workspace</FxButton>
      </form>
    </AuthLayout>
  );
}