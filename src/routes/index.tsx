  ...
  head: () => ({
    meta: [
      { title: "FixLoop AI — From Customer Pain to Verified Product Fixes" },
      {
        name: "description",
        content:
          "Transform thousands of support tickets into root-cause intelligence, deployment correlations, and verified product fixes.",
      },
    ...
    <header className="sticky top-0 z-30 border-b border-border bg-background/80 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center">
        ...
        <nav className="hidden md:flex items-center gap-7 ml-12 text-sm text-muted-foreground">
          ...
          <a href="#advantage" className="hover:text-foreground">
            Why FixLoop
          </a>
        </nav>
        ...
        <div className="grid lg:grid-cols-[1.1fr_1fr] gap-12 items-center">
          <div>
            ...
            <h1 className="mt-6 text-5xl md:text-6xl font-bold leading-[1.05] tracking-tight">
              Your support queue knows what's broken.
              <br />
              <span
            ...
            <p className="mt-6 text-lg text-muted-foreground max-w-xl">
              Transform thousands of support tickets into root-cause intelligence, deployment
              correlations, and verified product fixes — automatically.
            </p>
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <Link to="/dashboard">
                <FxButton size="lg">
                  Analyze Tickets <ArrowRight className="h-4 w-4" />
                </FxButton>
              </Link>
              <FxButton size="lg" variant="outline">
          ...
          <div className="relative">
            <div
              ...
              style={{ background: "var(--gradient-cyber)", filter: "blur(60px)", opacity: 0.2 }}
            />
            <div className="relative rounded-xl border border-border bg-card shadow-2xl overflow-hidden">
              <div className="flex items-center gap-1.5 px-3 py-2 border-b border-border bg-surface">
                ...
                <span className="ml-3 text-[11px] text-mono text-muted-foreground">
                  fixloop.ai / clusters
                </span>
              </div>
...
function Problem() {
  const items = [
    {
      icon: AlertOctagon,
      title: "Tickets pile up in silos",
      body: "Support, success, and engineering each see fragments. Nobody sees the pattern.",
    },
    {
      icon: Activity,
      title: "Root cause is guesswork",
      body: "By the time a trend is obvious, customers have already churned.",
    },
    {
      icon: GitBranch,
      title: "Deploys go uncorrelated",
      body: "Releases ship without ever knowing which ones caused the next wave of complaints.",
    },
  ];
  return (
      ...
      <div className="max-w-7xl mx-auto px-6 py-20">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-2 text-4xl font-bold tracking-tight">
            Companies treat tickets as support data. FixLoop treats them as product intelligence.
          </h2>
        </div>
          ...
          {items.map((it) => (
            <div key={it.title} className="rounded-lg border border-border bg-card p-6">
              <div
                ...
                style={{ background: "color-mix(in oklab, var(--critical) 14%, transparent)" }}
              >
  ...
  const steps = [
    {
      icon: Search,
      title: "Ingest",
      body: "Connect Zendesk, Intercom, Linear, GitHub, and your release feed. FixLoop normalizes every signal.",
    },
    {
      icon: Layers,
      title: "Cluster",
      body: "AI groups thousands of tickets into root-cause clusters with confidence scores and business impact.",
    },
    {
      icon: GitBranch,
      title: "Correlate",
      body: "Every deploy is time-aligned with downstream pain. See causal links you'd never find by hand.",
    },
    {
      icon: Wrench,
      title: "Resolve",
      body: "Recommended fixes, owners, and expected deflection. Closed loops with verified before/after.",
    },
  ];
  return (
    <section id="how" className="relative">
      <div className="max-w-7xl mx-auto px-6 py-24">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-2 text-4xl font-bold tracking-tight">
            Four steps from raw tickets to verified fixes.
          </h2>
        </div>
        <div className="mt-12 grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {steps.map((s, i) => (
            <div
              key={s.title}
              className="relative rounded-lg border border-border bg-card p-6 group hover:border-primary/40 transition-colors"
            >
              <div className="absolute top-4 right-4 text-[11px] text-mono text-muted-foreground">
                0{i + 1}
              </div>
              <div
    ...
    <section id="preview" className="relative border-y border-border bg-surface/50">
      <div className="max-w-7xl mx-auto px-6 py-24">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-2 text-4xl font-bold tracking-tight">
            An intelligence ops center, not a support inbox.
          </h2>
          <p className="mt-3 text-muted-foreground">
            High-information density. Built for product, engineering, and execs to see the same
            truth.
          </p>
        </div>
        ...
        <div className="mt-12 grid lg:grid-cols-3 gap-4">
          <FeatureCard
            ...
            body="Auto-discovered groups with severity, customer count, monthly cost, and confidence."
            to="/clusters"
          />
          <FeatureCard
            icon={Activity}
            title="Causal Timeline"
            body="Deploys, releases, and clusters on one timeline. Correlation lines tell the story."
            to="/timeline"
          />
          <FeatureCard
            icon={Wrench}
            title="Resolution Center"
            body="Recommended fixes with expected deflection, owners, and verified before/after."
            to="/resolution"
          />
          <FeatureCard
            icon={LineIcon}
            title="Health Dashboard"
            body="Revenue at risk, open clusters, customers affected, deflection rate — live."
            to="/dashboard"
          />
          <FeatureCard
            icon={Target}
            title="Priority Ranking"
            body="What to fix next, ranked by combined volume, severity, and revenue impact."
            to="/report"
          />
          <FeatureCard
            icon={Sparkles}
            title="Executive Briefs"
            body="Board-ready PDF reports generated from real ticket data, not hand-rolled slides."
            to="/report"
          />
        </div>
      </div>
...
function FeatureCard({ icon: Icon, title, body, to }: any) {
  return (
    <Link
      to={to}
      className="group rounded-lg border border-border bg-card p-6 hover:border-primary/40 transition-colors block"
    >
    ...
    <section id="advantage" className="relative">
      <div className="max-w-7xl mx-auto px-6 py-24">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-2 text-4xl font-bold tracking-tight">
            A help desk gives you tickets. FixLoop closes the loop.
          </h2>
        </div>
        ...
        <div className="mt-10 rounded-lg border border-border bg-card overflow-hidden">
          ...
          {rows.map((r, i) => (
            <div
              key={i}
              className="grid grid-cols-[1.4fr_1fr_1fr] border-b border-border/50 last:border-0 text-sm"
            >
              <div className="px-5 py-3 font-medium">{r[0]}</div>
              <div className="px-5 py-3 text-muted-foreground">{r[1]}</div>
              <div className="px-5 py-3 text-foreground flex items-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-6 pb-24">
        <div className="relative overflow-hidden rounded-2xl border border-border p-12 text-center">
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse at center, color-mix(in oklab, var(--primary) 22%, transparent), transparent 70%)",
            }}
          />
          <div className="relative">
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight max-w-3xl mx-auto">
              Close the loop on every complaint, every deploy, every dollar at risk.
            </h2>
            <p className="mt-4 text-muted-foreground max-w-xl mx-auto">
              Spin up FixLoop on your ticket history in under an hour. See the first cluster within
              minutes.
            </p>
    ...
    <footer className="border-t border-border bg-surface/40">
      <div className="max-w-7xl mx-auto px-6 py-10 flex flex-wrap items-center gap-4">
        ...
        <span className="text-xs text-muted-foreground text-mono">
          © 2026 FixLoop AI · Product Intelligence Platform
        </span>
