...
import { CheckCircle2, ArrowRight, TrendingDown, DollarSign, Loader2, Play, Wand2, Calculator, Bug, ShieldCheck, GitBranch, Activity, FileText } from "lucide-react";

import { useAllRecommendations, type RecommendationOut } from "@/hooks/useRecommendations";
import { useValidation, useRunValidationMutation, type ValidationSummary } from "@/hooks/useValidation";
...
function ResolutionPage() {
  const { data: recommendations = [], isLoading: isLoadingRecs } = useAllRecommendations();
      ...
      {selectedRec ? (
        ...
        <>
          <Panel title="Loop Closures" subtitle="Verified before/after ticket volume per shipped fix">
            {resolved.length === 0 ? (
              <div className="text-center text-muted-foreground py-8">No loop closures yet. Ship a fix and run validation to see it here.</div>
            ) : (
                ...
                {resolved.map((r) => (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="h-9 w-9 rounded-full bg-secondary/20 flex items-center justify-center cursor-pointer" onClick={() => setSelectedRecId(r.id)}>
                    ...
                    <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                      ...
                      <div className="rounded-md border border-secondary/40 bg-secondary/10 p-3 text-center">
                        ...
                        <div className="mt-1 text-2xl font-bold text-mono text-secondary">{r.after_ticket_count || 0}</div>
                        <div className="text-[10px] text-muted-foreground">tickets / mo</div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-mono">{r.actual_reduction_pct || r.expected_reduction_pct || 0}%</span> deflection
                      </div>
                      <div className="flex items-center gap-2">
                        <DollarSign className="h-4 w-4 text-primary" />
                        <span className="font-semibold text-mono text-primary">
                          ${((r.actual_recovery_usd || r.expected_recovery_usd || 0) / 1000).toFixed(1)}k
                        </span>{" "}
                        / mo
                      </div>
                    </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {recommendations.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-8 text-center text-muted-foreground">No recommendations generated yet. Head to the AI Command Center to investigate clusters.</td>
                    </tr>
                  ) : recommendations.map((r) => (
                    <tr key={r.id} className="border-b border-border/50 align-top hover:bg-accent/30 cursor-pointer" onClick={() => setSelectedRecId(r.id)}>
                      <td className="py-3 pr-3">
                        <div className="font-medium text-primary hover:underline">{r.title}</div>
                        <div className="text-xs text-muted-foreground text-mono">{r.cluster_id} · {r.id.split("-")[0]}</div>
                      </td>
                      <td className="py-3 pr-3 text-mono text-xs">{r.engineering_effort?.replace("_", " ")}</td>
...
function RecommendationDetail({ rec, onBack }: { rec: RecommendationOut, onBack: () => void }) {
  ...
  const { mutate: runValidation, isPending: isValidating, error: validationError } = useRunValidationMutation();
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="mt-2 flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-xs text-muted-foreground text-mono">Effort: {rec.engineering_effort}</span>
                  <span className="text-xs text-muted-foreground text-mono text-primary">Confidence: {rec.confidence_score}%</span>
                </div>
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-border">
                ...
                <div className="rounded-md border border-border bg-surface p-5">
                  ...
                  <div className="mt-2 text-sm text-muted-foreground whitespace-pre-wrap">{rec.jira_description || rec.description}</div>
                  
                  {rec.jira_acceptance_criteria && rec.jira_acceptance_criteria.length > 0 && (
                    <div className="mt-4">
                      <div className="text-xs font-semibold uppercase tracking-wider text-foreground mb-2">Acceptance Criteria</div>
                  ...
                  <div className="mt-4 flex gap-2">
                    <FxButton size="sm" variant="cyber"><Bug className="h-3.5 w-3.5"/> Create in Jira</FxButton>
                  </div>
                </div>
              </div>
            </div>
              ...
              {isValidating ? (
                <div className="flex flex-col items-center justify-center p-8 space-y-4">
                  <div className="relative">
                    <div className="absolute inset-0 rounded-full animate-ping opacity-20" style={{ background: "var(--secondary)" }} />
                    <div className="h-12 w-12 rounded-full flex items-center justify-center relative z-10" style={{ background: "linear-gradient(135deg, var(--secondary), var(--accent))" }}>
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <div className="text-xs text-muted-foreground animate-pulse">Building summary</div>
                  </div>
                </div>
              ) : validation ? (
                <>
                  ...
                  <div className="pt-4 flex gap-2 border-t border-border">
                    <FxButton size="sm" variant="outline" className="w-full" onClick={handleValidate}>
                      Re-run Validation
                    </FxButton>
                  </div>
                </>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-muted-foreground mb-6">
                    Has this fix been shipped? Run the validation engine to measure actual deflection and loop closure.
                  </p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-md border border-border bg-surface p-3">
          ...
          <div className="mt-1 text-xl font-bold text-[color:var(--critical)] text-mono">{validation.before_ticket_count}</div>
          <div className="text-[10px] text-muted-foreground">tickets / mo</div>
          <div className="mt-2 text-xs font-semibold text-[color:var(--critical)]">${(validation.before_revenue_risk / 1000).toFixed(1)}k risk</div>
        </div>
        <div className="rounded-md border border-secondary/30 bg-secondary/10 p-3">
          ...
          <div className="mt-1 text-xl font-bold text-secondary text-mono">{validation.after_ticket_count}</div>
          <div className="text-[10px] text-muted-foreground">tickets / mo</div>
          <div className="mt-2 text-xs font-semibold text-secondary">${(validation.after_revenue_risk / 1000).toFixed(1)}k risk</div>
        </div>
      </div>

      <div className="rounded-md border border-primary/30 bg-primary/5 p-4 text-center">
        <div className="text-[10px] uppercase tracking-wider text-primary font-semibold mb-1">Deflection Achieved</div>
        ...
        <div className="mt-3 text-sm text-muted-foreground">
          Recovered <span className="text-foreground font-mono font-bold">${validation.revenue_recovered_usd.toLocaleString()}</span> / mo
        </div>
      </div>
    </div>
  );
