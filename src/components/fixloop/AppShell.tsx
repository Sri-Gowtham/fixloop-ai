...
export function AppShell() {
  ...
  // Derive a workspace label from the user's email domain
  const workspaceLabel = email !== "No email"
    ? email.split("@")[1]?.split(".")[0] ?? "My Workspace"
    : "My Workspace";
        ...
        <div className="px-3 py-3 border-b border-sidebar-border">
          <div className="flex items-center justify-between rounded-md border border-sidebar-border bg-sidebar-accent/40 px-2.5 py-1.5">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="text-xs font-medium truncate capitalize">{workspaceLabel}</div>
            </div>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                title={displayName}
                className="h-8 w-8 rounded-md text-[11px] font-bold flex items-center justify-center text-mono cursor-pointer hover:opacity-80 transition-opacity"
                style={{ background: "var(--gradient-cyber)", color: "var(--primary-foreground)" }}
              >
                {initials}
              </button>
              {menuOpen && (
                <>
                  ...
                  />
                  <div className="absolute right-0 top-10 z-40 w-56 rounded-lg border border-border bg-card shadow-2xl overflow-hidden">
                    <div className="px-4 py-3 border-b border-border">
                      <div className="text-sm font-semibold truncate">{displayName}</div>
                      <div className="text-xs text-muted-foreground text-mono truncate">{email}</div>
                    </div>
                    <div className="p-1">
                      <Link
                        to="/profile"
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2.5 rounded-md px-3 py-2 text-sm text-critical hover:bg-critical/10 transition-colors"
                      >
