    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h3 className="mt-1.5 text-base font-semibold leading-tight tracking-tight">{cluster.title}</h3>
      ...
      <div className="mt-4 grid grid-cols-3 gap-3 border-t border-border pt-4">
        <Stat icon={DollarSign} label="Monthly cost" value={`$${(cluster.monthlyCost / 1000).toFixed(1)}k`} />
      ...
      <div className="mt-3 flex items-center justify-between text-xs">
        <span className="text-muted-foreground text-mono">{cluster.relatedDeploy ? `Linked to ${cluster.relatedDeploy}` : "No deploy correlation"}</span>
